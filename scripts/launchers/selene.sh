...
: "${NRUNS:=1}"
: "${NEXP:=1}"
: "${CONFIG:=config_DGXA100_1x8x2.sh}"
: "${BASE_LOG_DIR:=logs}"
: "${JOB_NAME:=job}"
: "${CONTAINER_IMAGE:=gitlab-master.nvidia.com/akiswani/images/mlperf_retinanet_pytorch:21.09-py3}"
...
for i in $(seq 1 "${NRUNS}"); do
    ...
    sbatch \
        ...
        ./scripts/launchers/srun.sh "${CONTAINER_IMAGE}"
    sleep 1
done
echo ""
